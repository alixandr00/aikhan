...
export const Forma = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Используем displayName или email в качестве запасного варианта
      const userInfo = {
        displayName: user.displayName || user.email, // Используем email если displayName отсутствует
        email: user.email,
      };
      ...
      // Проверка на специальные учетные данные администратора
      if (
        loginEmail === "aktilek@gmail.com" &&
        loginPassword === "aktilekSILA"
      ) {
        console.log("Администратор вошел в систему.");
        navigate("/admin-page"); // Навигация на страницу админа
      } else {
